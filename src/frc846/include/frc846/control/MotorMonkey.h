...
public:
  ...
  */
  ...
  static void WriteVoltage(size_t slot_id, units::volt_t voltage);
  /*
  WriteVelocity()
...
private:
  ...
  struct MotorMessage {
    enum class Type { DC, Position, Velocity, Voltage };
    size_t slot_id;
    Type type;
    std::variant<double, units::radian_t, units::radians_per_second_t,
        units::volt_t>
        value;
  };
