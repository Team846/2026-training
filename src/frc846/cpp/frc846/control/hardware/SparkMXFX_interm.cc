...
void SparkMXFX_interm::Tick() {
  ...
  if (double* dc = std::get_if<double>(&last_command_)) {
    ...
    if (excessive_current_past_loops > 0)
      ...
      double voltage_normalized = dc_u * 12.0;
      last_status_code = pid_controller_->SetReference(
          voltage_normalized, rev::spark::SparkBase::ControlType::kVoltage);
    }
    ...
    double voltage_normalized = volt->to<double>() / 12.0;
    last_status_code = pid_controller_->SetReference(
        voltage_normalized, rev::spark::SparkBase::ControlType::kVoltage);
  }
...
void SparkMXFX_interm::WriteVoltage(units::volt_t voltage) {
  last_command_ = voltage;
}

bool SparkMXFX_interm::IsDuplicateControlMessage(units::volt_t voltage) {
