...
void TalonFX_interm::Tick() {
  ...
  if (double* dc = std::get_if<double>(&last_command_)) {
    ctre::phoenix6::controls::VoltageOut dc_msg =
        units::make_unit<units::volt_t>(*dc * 12.0);
  ...
  } else if (units::volt_t* volt = std::get_if<units::volt_t>(&last_command_)) {
    ctre::phoenix6::controls::VoltageOut voltage_msg{*volt};
    voltage_msg.WithUpdateFreqHz(0_Hz);
    last_status_code = talon_.SetControl(voltage_msg);
  }
...
bool TalonFX_interm::IsDuplicateControlMessage(units::volt_t voltage) {
  if (units::volt_t* volt = std::get_if<units::volt_t>(&last_command_)) {
    return *volt == voltage;
  }
  return false;
}

void TalonFX_interm::WriteVoltage(units::volt_t voltage) {
  last_command_ = voltage;
}
