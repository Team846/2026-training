...
void MCSimulator::WriteVoltage(units::volt_t voltage) {
  double duty_cycle = voltage / battery_voltage_;
  duty_cycle = std::clamp(duty_cycle, -1.0, 1.0);
  if (inverted) duty_cycle = -duty_cycle;
  control_message = duty_cycle;
}
...
bool MCSimulator::IsDuplicateControlMessage(units::volt_t voltage) {
  if (inverted) voltage = -voltage;
  return std::holds_alternative<units::volt_t>(control_message) &&
         std::get<units::volt_t>(control_message) == voltage;
}
