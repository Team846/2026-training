...
units::ampere_t MotorMonkey::WriteMessages(units::ampere_t max_draw) {
  ...
  while (!temp_messages.empty()) {
    ...
    case MotorMessage::Type::DC:
      ...
      break;
    case MotorMessage::Type::Voltage:
      duty_cycle = std::get<units::volt_t>(msg.value) / battery_voltage;
      break;
  ...
  while (!control_messages.empty()) {
    ...
    case MotorMessage::Type::Voltage: {
      auto voltage = std::get<units::volt_t>(msg.value);
      if (!controller->IsDuplicateControlMessage(voltage) ||
          controller->GetLastErrorCode() !=
              frc846::control::hardware::ControllerErrorCodes::kAllOK) {
        controller->WriteVoltage(voltage);
        LOG_IF_ERROR("WriteVoltage");
      }
      break;
    }
    case MotorMessage::Type::Position: {
...
void MotorMonkey::WriteVoltage(size_t slot_id, units::volt_t voltage) {
  CHECK_SLOT_ID();

  control_messages.push({slot_id, MotorMessage::Type::Voltage, voltage});
}
